...
class CustomCalendarDatePicker extends StatefulWidget {
  ...
  final Map<String, Color>? daysColors;
...
class _DayPicker extends StatefulWidget {
  ...
  /// The currently selected date.
  ///
  /// Custom Background Color For The Day Widget
  final Map<String, Color>? daysColors;
    ...
    while (day < daysInMonth) {
      ...
      if (day < 1) {
        ...
        final Color? dayForegroundColor = resolve<Color?>(
            ...
            states);
        Color? dayColor() {
          if (widget.daysColors == null || widget.daysColors!.isEmpty) {
            return null;
          }
          final dayKey = intl.DateFormat('yyyy-MM-dd')
              .parse(dayToBuild.toString())
              .toString()
              .split(' ')[0];
          if (!widget.daysColors!.keys.toList().contains(dayKey)) {
            return null;
          }
          return widget.daysColors![dayKey];
        }

        final Color? dayBackgroundColor = resolve<Color?>(
            (DatePickerThemeData? theme) => dayColor() != null
                ? MaterialStatePropertyAll(dayColor())
                : isToday
        ...
        final BoxDecoration decoration = isToday
            ? BoxDecoration(
                color: isSelectedDay ? AppColors.primary : dayBackgroundColor,
                border: Border.fromBorderSide(
                    (datePickerTheme.todayBorder ?? defaults.todayBorder!)
                        .copyWith(color: dayForegroundColor, width: 1)),
          ...
          child: Center(
            child: Text(
              ...
              style: dayStyle?.apply(
                color: dayColor() != null ? Colors.white : dayForegroundColor,
              ),
            ),
          ),
        );
